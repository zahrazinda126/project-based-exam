    ...
    async function fetchData() {
      try {
        ...
        if (trendRes.status === "fulfilled") {
          setTrending(Array.isArray(trendRes.value.results) ? trendRes.value.results : []);
        }
        if (npRes.status === "fulfilled") {
          setNowPlaying(Array.isArray(npRes.value.results) ? npRes.value.results : []);
        }
        if (trRes.status === "fulfilled") {
          setTopRated(Array.isArray(trRes.value.results) ? trRes.value.results : []);
        }
      } catch (err) {
